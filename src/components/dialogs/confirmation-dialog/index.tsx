      ...
      <Dialog fullWidth maxWidth='xs' open={open} onClose={() => setOpen(false)} closeAfterTransition={false}>
        <DialogContent className='flex items-center flex-col text-center sm:pbs-16 sm:pbe-6 sm:pli-16'>
          ...
          >
            ...
            {type === 'suspend-account' && <Typography color='text.primary'>You won&#39;t be able to revert user!</Typography>}
            {type === 'delete-order' && <Typography color='text.primary'>You won&#39;t be able to revert order!</Typography>}
            {type === 'delete-customer' && <Typography color='text.primary'>You won&#39;t be able to revert customer!</Typography>}
          </Wrapper>
        </DialogContent>
        <DialogActions className='justify-center pbs-0 sm:pbe-16 sm:pli-16'>
          <Button variant='contained' onClick={() => handleConfirmation(true)}>
            {type === 'suspend-account' ? 'Yes, Suspend User!' : type === 'delete-order' ? 'Yes, Delete Order!' : type === 'delete-customer' ? 'Yes, Delete Customer!' : 'Yes'}
          </Button>
          <Button
            variant='tonal'
            color='secondary'
